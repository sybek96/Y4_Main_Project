...
// moved toward the new position in a linearly interpolated fashion.
// while player is moving a vignette effect is applied to the camera
// in order to reduce peripheral vision and therefore reducing motion sickness.
//
//=============================================================================
...
{
    //-------------------------------------------------------------------------
    public class PointAndDash_mine : MonoBehaviour
    {
        ...
        private PostProcess cameraScript;
        private Material cameraPostProcessMaterial;
        ...
        //-------------------------------------------------
        private static PointAndDash_mine _instance;
        public static PointAndDash_mine instance
        {
            get
            {
                if (_instance == null)
                {
                    _instance = GameObject.FindObjectOfType<PointAndDash_mine>();
                }
        ...
        void Update()
        {
            ...
            if (currentlyDashing)
            {
                //lerp player toward the new position if they are outside the area to teleport to.
                cameraScript.enabled = true;
                if ((player.trackingOriginTransform.position.x > dashDestination.x + 0.0001f || player.trackingOriginTransform.position.x < dashDestination.x - 0.0001f)
                    && 
                    (player.trackingOriginTransform.position.z > dashDestination.z + 0.0001f || player.trackingOriginTransform.position.z < dashDestination.z - 0.0001f))
                {
                    player.trackingOriginTransform.position = Vector3.Lerp(player.trackingOriginTransform.position, dashDestination, 0.1f);
                }
                else
                {
                    //when the player is finished dashing disable vignette
                    cameraScript.enabled = false;
